...
| 超过计划时间一倍 | **0 分** |
| 勾了「超期不扣分」 | 超时多久都**拿全额**（如「陪家人聊聊天」） |
| 勾了「严格限时」 | 只要超时**立刻 0 分**（如「朗读英语 10 分钟」） |
...
node scripts/e2e-check.mjs               # 16 项：渲染 / 导航 / 溢出 / 导出
node scripts/e2e-gameplay.mjs            # 21 项：结算规则 / 账本 / 农场 / 签到
```
