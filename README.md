...
#### 默认任务的积分口径（2026-09-21 整体减半）

首次启动播种的任务清单在 `src/domain/seedTasks.ts`，由 `seedTasks.test.ts` 逐档钉住：

| 档位 | 基础分 | 质量分 | 减半前 |
| --- | --- | --- | --- |
| 6 个学科任务（各 45 分钟） | 10 | +4 | 20 / +8 |
| 清理自己的房间（15 分钟） | 6 | +3 | 12 / +6 |
| 签到（练字 / 日记 / 晨读） | 4 | — | 8 |
| 读一本完整的故事书（月度） | 15 | +8 | 30 / +15 |
| 学会一项新本领（年度） | 50 | +25 | 100 / +50 |

于是每日任务收入从 132 分（打满质量分 186）降到 **66 分（93）**。

> ⚠️ **兑换定价和农场经济没有跟着减半 —— 这是有意的，不是漏改。**
> 后果：孩子攒钱速度大致减半（「大件愿望」从攒一周变成攒两周左右），
> 收入结构变成**农场占大头**（66 vs 100~200）。这是把经济**收紧**了。
> 若只想让数字变小、不想动难度，就得把兑换品 `cost` 和农场 `seedCost` /
> `cost` / 解锁价一起减半。完整推导见 `seedTasks.ts` 的定价锚点注释。
...
> ⚠️ 阶梯奖励的数值在 `domain/recurrence.ts` 的 `defaultTiers()` 里，是**规则模板**
> （任何签到任务都用），**没有跟着 2026-09-21 的减半动** —— 三个签到任务加起来
> 约 300 分/周 ≈ 43 分/天，不是小数目。要不要一起减半待家长确认。
...
- 连续完成有 🔥 连击奖励，每天递增、有上限
- **计时器开着，「直接填时间」那个旁路按钮藏着**（2026-09-21）：任务卡上是
  「▶️ 开始」的实时秒表，点开始后卡片换成「✅ 我做完啦！」→ 点开提交。
  ⚠️ 「开始」的**尺寸和位置有两条定论**（用户 2026-09-21 明确要求，别顺手改）：
  ① **保持 `size="md"` 的内容宽度**，不许改成 `full` 撑满整宽；
  ② **靠右对齐** —— 和计时中的「我做完啦」、补做的「现在补做掉」是同一个位置，
  状态一变按钮不跳；而它只占内容宽，左边那片空由元信息那列自然收着，不显得行没占满。
  （第一版两条都踩了：先改成撑满整宽，用户回「保持跟之前的大小呀」；
  再改回左对齐，用户回「位置别扭，UI 差点味道」。）
  两个开关都在 `src/features/tasks/ui.tsx`：
  `TIMER_ENABLED`（计时器本身，现为 `true`）、
  `MANUAL_FILL_ENABLED`（「▶️ 开始」右边那个「没用计时器 · 直接填时间」，现为 `false`）。
  ⚠️ **这里有一段误读要记住。** 用户原话是「『没用计时器，直接填时间』暂时可以隐藏。
  UI 记得调整 比如开始按钮」—— 要藏的是「▶️ 开始」**右边那个按钮**，
  不是整个计时器。上一版断错了句，把整块计时器关了，「开始」跟着一起消失，
  用户第二天就来问「为啥任务没有开始按钮了，直接就是做完了」。
  ⚠️ 藏掉旁路按钮要**连带补一处**：`rejected`（家长打回、让孩子重做）的卡片
  原来只有那个按钮能打开提交弹层 —— `startTimer()` 对非 `pending` 的实例是
  **直接 `return`** 的，所以那张卡上的「▶️ 开始」本来就是个点了没反应的死按钮。
  只藏不补，重做任务就永远交不上去。现在重做卡片直接给「✅ 我做完啦！」。
  ⚠️ 同理，卡片和详情页的 `running` 都**必须带 `status === 'pending'`**：
  打回时会保留 `startedAt`，不卡 pending 的话重做卡片会挂着一个从**上一次**
  开始算起的秒表（可能已经走了好几天），孩子一提交就是 0 分。
- 没用计时器的任务（重做 / 补做）在详情页填用时：`+/-` 步进器，
  或「按计划 / 两倍」快捷键
- 漏填或填错了可以事后「补做」
...
- 家长设置：孩子昵称头像、积分规则开关、**自定义一天从几点开始**（孩子熬夜到 1 点
  做的作业仍算"昨天"）、**现金对积分参考汇率**、家长密码锁

#### 现金对积分（参考汇率，2026-09-21）

家长定一个「1 块钱 ≈ 多少积分」（默认 **1 元 = 10 分**），兑换页每件商品旁边就会
标一句「≈ ¥3」。**纯展示，不影响任何数值** —— 扣多少分永远看标价本身。

口径与格式化在 `src/domain/cash.ts`（`formatYuan` / `clampPointsPerYuan`），
合法区间 `[1, 1000]`，缺字段自动回落默认档（老库不需要迁移）。
grep `pointsPerYuan` 只应命中三处：设置页（编辑）、兑换页（展示）、`cash.ts`。

**「不影响数值」这条落成了可执行的保证**，不是只写在注释里：

- 单测（`useApp.test.ts`）：改汇率**不动余额**，且兑换扣的分**等于标价本身**。
- E2E（`e2e-check.mjs` §8b，5 项）：点预设 / 自己填 → 真的落库；兑换页每件商品
  都标了「≈ ¥X」且 **X = 标价 ÷ 汇率**（脚本**独立重算**一遍，不 import 被测实现）。
  ⚠️ 自定义输入是「草稿 + 失焦提交」，用例特意断言**失焦前不落库** ——
  这条做过变异验证（把 `onCommit` 摘掉，检查立刻变红 78/79），不是空断言。
...
产物：`android/app/build/outputs/apk/debug/app-debug.apk`（约 5.5 MB）
...
node scripts/e2e-check.mjs               # 79 项：新手引导 / 渲染 / 导航 / 溢出 / 顶栏 / 长期任务孩子端 / 音效开关 / 币种只在一处显示 / 现金对积分汇率 / 市场走势图 / 导出
...
node scripts/e2e-regressions.mjs         # 28 项：**回归守卫** —— 已经修过的具体 bug 别再改回去
node scripts/e2e-wipe.mjs                # 18 项：全量清空（内存 + IndexedDB 都要清干净，清完重新播种）
```
...
```bash
node scripts/capture.mjs                 # → screenshots/redesign/，30 张
```
