...
/**
 * 实时计时器**暂时隐藏**（2026-09-21）。
 *
 * 用户原话：「没用计时器，直接填时间 暂时可以隐藏。UI 记得调整 比如开始按钮。」
 *
 * 关掉之后孩子看到的是：
 *   · 任务卡：不再有「▶️ 开始」，只剩一个「✅ 我做完啦！」→ 点开填用时
 *   · 详情页：不再有「正在计时中…」的实时秒表，直接给 +/− 步进器
 *
 * ⚠️ **只关 UI，不动数据。** `startTimer()` / `inst.startedAt` / `actualMinutes`
 * 原样保留 —— 孩子照样要填用时，超时扣分、准时奖励、家长审核这些规则
 * 一点都不变。所以把这里改回 `true` 就能整个恢复，不用动别的地方。
 *
 * 为什么用开关而不是直接删：用户说的是「**暂时**可以隐藏」。
 *
 * ⚠️ **但要清楚它的代价**：关着的时候，`true` 那条分支**没有任何测试会跑到**
 * （e2e 与单测都只走「填时间」这条路），放久了会烂 —— 等真要用时打开，
 * 很可能是坏的。**真要长期不用，应该连同 `startTimer` 一起删干净**，
 * 而不是让这段代码挂在这儿。这个开关的定位是「几周内会回来」，不是「长期配置」。
 *
 * 类型写成 `boolean`（不是 `= false` 的字面量）：否则 TS 会把
 * `TIMER_ENABLED ? A : B` 收窄成只剩 `B`，`A` 里的变量会被报成未使用。
 */
export const TIMER_ENABLED: boolean = false
