...
export function GrowthBar({
  ...
  return (
    <div
      className={clsx(
        'h-2.5 w-full overflow-hidden rounded-full border-2 border-ink-900/10 bg-ink-100/70',
        className,
      )}
      role="progressbar"
      aria-valuemin={0}
...
export function CoinPill({
  ...
  return (
    <span
      className={clsx(
        'tnum inline-flex items-center gap-1 rounded-full border-2 px-2.5 py-0.5 font-display text-sm font-extrabold',
        skin,
        className,
      )}
    >
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      <button
        ...
        onClick={onClose}
      />
      <div className="anim-sheet-up relative flex max-h-[88vh] flex-col overflow-hidden rounded-t-[2rem] border-t-[3px] border-ink-900/10 bg-paper shadow-float">
        {/* 抓手 */}
        ...
        <div className="flex items-center gap-3 px-5 pb-2 pt-2">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="关闭"
            className="btn-3d grid size-11 shrink-0 place-items-center rounded-full border-[3px] border-ink-900/10 bg-white text-lg text-ink-700 active:btn-3d-press"
          >
            ✕
          </button>
        </div>
