/* ============================================================
   ...
   `backgroundColor` 要跟着改（现为 `#caac65`），否则原生启动图会和这一页闪一下。
   取值口径见下面「天光」那段注释 —— 是**首帧整幅的平均色**，不是渐变某一站的色值。
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-6">
      ...
      {/* 天光：一层暖色暗罩，随太阳升起淡出（keyframes 见 theme.css）。
          ...
          SplashScreen.backgroundColor）只能填一个颜色。这里填的是
          **首帧整幅的平均色**（现为 `#caac65`，实测），不是渐变某一站的色值 ——
          整屏铺一个颜色，用平均色接上去最不容易看出接缝。
          **改这里的颜色或透明度，必须重新量一次首帧平均色并同步那个值。**
